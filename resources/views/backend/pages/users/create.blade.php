...
<form class="relative mb-32" style="height: auto;" action="{{ route('users.store') }}" method="post">
    @csrf
        ...
        <div>
            ...
            <div class="flex flex-wrap -mx-3">
                ...
                <div class="w-full max-w-full px-3 mt-4 flex-0 sm:mt-0 sm:w-6/12">
                    <label class="mb-2 ml-1 font-bold text-xs text-slate-700 dark:text-white/80" for="email">Email Address</label>
                    <input type="email" name="email" placeholder="eg. user@example.com" class="focus:shadow-soft-primary-outline dark:bg-gray-950 dark:placeholder:text-white/80 dark:text-white/80 text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none">
                </div>
            </div>
                ...
                <div class="w-full max-w-full px-3 mt-4 flex-0 sm:mt-0 sm:w-6/12">
                    ...
                    <select name="Role" class="focus:shadow-soft-primary-outline dark:bg-gray-950 dark:placeholder:text-white/80 dark:text-white/80 text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none">
                        ...
                        @foreach ($roles as $item)
                           <option value="{{ $item->name }}">{{ $item->name }} - {{ $item->guard_name }}</option>
                       @endforeach
                    </select>
                </div>
            </div>
